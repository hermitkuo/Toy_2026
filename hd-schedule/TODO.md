...
### 待辦 — 國定假日表（手動，因無 OAuth 寫入權）
- [~] 已產生官方版國定假日清單 `national_holidays_2023-2026.csv`（74 筆，來源：TW 政府行政機關辦公日曆表 2023–2026；欄＝日期／節日名稱／醫院是否放假）。
      *.csv 被 global gitignore 排除，檔案在 repo 目錄供貼上用。
- [ ] 使用者：把該 CSV 貼進假日試算表 gid=592318403
      （https://docs.google.com/spreadsheets/d/1UdSMWQG1XxpNa7jRRmvK5cqQ7sfMb7E4O89B-y5Te9c ），
      填「醫院是否放假」欄（否/沒放假/上班→不計；空白或是→計入），並設「知道連結的人可檢視」。
      頁面 stat 模組 HOL_SHEET/HOL_GID 已對準此表，填好即自動顯示。
